...
def verifyFunction (lMod:LLVM.Module) (fAnn: FunctionAnn): ModuleVCG FnVerification := do
  ...
  let blockVCs : Array BlockVerification := #[]
  let blockErrors : Array BlockError := #[]
...
def reportErrors (bErrors : List BlockError) (mErrors : List ModuleError) (success : Bool) : IO UInt32 := do
  ...
  if !bErrors.isEmpty then do
    IO.println ""
    IO.println "======================= ERRORS ======================="
    let bErrMap : Std.RBMap BlockErrorTag (Nat × (Std.RBMap String Nat (·<·))) (·<·) := Std.RBMap.empty
  ...
  if !mErrors.isEmpty then do
    let mErrMap : Std.RBMap ModuleErrorTag (Nat × (Std.RBMap String Nat (·<·))) (·<·) := Std.RBMap.empty
