...
instance less : Less BlockErrorTag :=
...
instance less : Less ModuleErrorTag :=
  ⟨λ x y => x.index < y.index⟩
...
-- Table header row for summarizing a series of VerificationResults,
-- N.B., should be equal in length to lists produced by `toTableRow`.
def tableHeaderRow : List String :=
  ["LLVM Function Name", 
   "LLVM Block Label",
   "LLVM Instruction Index",
   "Machine Code Address",
   "Goal Tag",
   "Goal Extra Info",
...
protected def toJson (vr : VerificationResult) : Lean.Json :=
  Lean.Json.mkObj 
    [("LLVM Function Name", vr.goal.loc.fnName),
     ...
     ("LLVM Instruction Index", repr vr.goal.loc.llvmInstrIdx),
